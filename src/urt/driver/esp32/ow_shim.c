...
// Counter entry points hand D a bare uint32_t error code, never a by-value struct:
// LDC returns a one-word struct through a hidden pointer where GCC returns it in a
// register, which shifts every argument by one. D wraps the code back into a Result.
