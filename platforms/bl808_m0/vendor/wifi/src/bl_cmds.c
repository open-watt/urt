...
// (Vendor had #undef'd bl_os_log_printf to silence cmd-path chatter; re-enabled
// here for bring-up so we can see push/wait/CFM events.)

static void cmd_dump(const struct bl_cmd *cmd)
{
    // RWNX_ID2STR -> bl_id2str string tables aren't in our kept vendor sources;
    // drop the rich dump and just print the numeric fields.
    bl_os_log_debug("cmd_dump: tkn=%d flags=%04x result=%d id=%d reqid=%d\n",
...
static int cmd_mgr_msgind(struct bl_cmd_mgr *cmd_mgr, struct ipc_e2a_msg *msg, msg_cb_fct cb)
{
    ...
    /* The LMAC blob writes msg->id as 32 bits but only the low 16 bits hold
     * the semantic message id; the high bits look like an auxiliary
     * task/version tag. Probe shows id = 0x00090001 for MM_RESET_CFM (low
     * 16 = 0x0001 matches MM_RESET_CFM = 1, high 16 = 0x0009). Mask both
     * sides to 16 bits before comparison. */
    uint16_t msg_id_low = (uint16_t)(msg->id & 0xFFFF);

    bl_os_mutex_lock(cmd_mgr->lock);
    list_for_each_entry(cmd, &cmd_mgr->cmds, list) {
        if ((uint16_t)(cmd->reqid & 0xFFFF) == msg_id_low &&
            (cmd->flags & RWNX_CMD_FLAG_WAIT_CFM)) {
