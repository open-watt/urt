...
import urt.driver.ethernet : EthernetConfig, EthLinkCallback, EthLinkEvent, EthLinkInfo, EthMac, EthReadyCallback,
                             EthRxCallback, EthRxInfo, EthSpeed, EthSwitchLinkCallback;
...
enum bool has_eth_switch = true;

enum uint num_front_ports = 5;

const(char)[] eth_hw_name(uint port)
    => "ge1";

uint eth_hw_switch_ports(uint port)
    => num_front_ports;
...
void fe_init()
{
    version (RouterBoot)
        ...
        static bool board_mac(ref ubyte[6]) => false;
    ...
    _fq = cast(uint*)uncached(base + fq_ring_off);
    ...
    {
        _fq[i * 4 + 0] = fq_buf_phys + i * fq_page;
        _fq[i * 4 + 1] = fq_phys + (i + 1) * 16;
        _fq[i * 4 + 2] = fq_page << 16;
        _fq[i * 4 + 3] = 0;
    }
...
bool eth_hw_close(uint port)
{
    ...
    foreach (sp; 0 .. num_front_ports)
        eth_hw_switch_port_enable(port, sp, false);
    return true;
}

// The conduit carries only tagged traffic; frames go out a front port or not at all.
bool eth_hw_tx(uint port, const(ubyte)[] frame, bool insert_checksum)
    => false;

bool eth_hw_tx_switch(uint port, const(ubyte)[] frame, uint switch_port)
    => fe_tx_tagged(frame, 1u << switch_port);
...
// The conduit to the CPU port never changes state.
void eth_hw_set_link_callback(uint port, EthLinkCallback cb)
{
}

void eth_hw_set_switch_link_callback(uint port, EthSwitchLinkCallback cb)
{
    _switch_link_callback = cb;
}
...
bool eth_hw_service(uint port, size_t budget)
{
    ...
    while (n < budget)
    {
        ...
        if (len < 14 || _rx_callback is null || !(d2 & rxd2_vtag) || source >= num_front_ports)
            ++_rx_drops;
        else
        {
            EthRxInfo info;
            info.switch_port = cast(ubyte)source;
            _rx_callback(EthMac(cast(ubyte)port), (_rx_buf + i * rx_buf_size)[0 .. len], info);
        }
        d[1] = rx_buf_size << 16;
        _rx_calc = i;
        ++n;
    ...
    import urt.driver.mt7621.timer : mtime_read;
    immutable now = mtime_read();
    if (now - _links_polled >= link_poll_us)
    {
        _links_polled = now;
        poll_switch_links(port);
    }
...
{
    immutable dropped = _rx_drops;
    _rx_drops = 0;
    return dropped;
}

bool eth_hw_switch_port_enable(uint port, uint switch_port, bool enable)
{
    uint pcr;
    if (!sw_read(mt7530_pcr(switch_port), pcr) || !sw_write(mt7530_pcr(switch_port), (pcr & ~pcr_matrix_mask) | (enable ? pcr_matrix(1 << cpu_port) : 0)))
        return false;
    immutable bit = 1u << switch_port;
    _enabled = enable ? _enabled | bit : _enabled & ~bit;
    if (!enable && (_link_up & bit))
    {
        _link_up &= ~bit;
        if (_switch_link_callback !is null)
            _switch_link_callback(EthMac(cast(ubyte)port), cast(ubyte)switch_port, EthLinkEvent.down);
    }
    return true;
}

bool eth_hw_get_switch_link(uint port, uint switch_port, ref EthLinkInfo info)
{
    uint pmsr;
    if (!sw_read(mt7530_pmsr(switch_port), pmsr) || !(pmsr & pmsr_link))
        return false;
...
enum ulong link_poll_us = 1_000_000;
...
__gshared uint _rx_drops;
__gshared ulong _links_polled;
__gshared EthRxCallback _rx_callback;
__gshared EthSwitchLinkCallback _switch_link_callback;
__gshared EthReadyCallback _ready_callback;

uint phys(uint kseg0) => kseg0 & 0x1FFF_FFFF;
uint uncached(uint kseg0) => phys(kseg0) | 0xA000_0000;
uint tx_desc_phys(uint i) => _tx_phys + i * 16;

uint pcr_matrix(uint ports) => ports << 16;
uint mt7530_pcr(uint p) => 0x2004 + p * 0x100;
uint mt7530_psc(uint p) => 0x200C + p * 0x100;
uint mt7530_pvc(uint p) => 0x2010 + p * 0x100;
uint mt7530_ppbv1(uint p) => 0x2014 + p * 0x100;
uint mt7530_pmsr(uint p) => 0x3008 + p * 0x100;

void spin(uint us)
{
    import urt.driver.mt7621 : spin_us;
    spin_us(us);
}

// Front ports start isolated: no forwarding and no learning until a port is enabled.
bool switch_init()
{
...
void poll_switch_links(uint port)
{
    foreach (sp; 0 .. num_front_ports)
    {
        immutable bit = 1u << sp;
        if (!(_enabled & bit))
            continue;
        uint pmsr;
        if (!sw_read(mt7530_pmsr(sp), pmsr))
            continue;
        ...
        if (_switch_link_callback !is null)
            _switch_link_callback(EthMac(cast(ubyte)port), cast(ubyte)sp, up ? EthLinkEvent.up : EthLinkEvent.down);
    }
}
