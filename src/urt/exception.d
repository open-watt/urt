/// Assert handler registration for uRT.
module urt.exception;

import urt.attribute : noinline;

nothrow @nogc:


alias AssertHandler = void function(string file, size_t line, string msg) nothrow @nogc;

AssertHandler assert_handler() @property nothrow @nogc @trusted
    => _assert_handler;

void assert_handler(AssertHandler handler) @property nothrow @nogc @trusted
{
    if (handler is null)
        _assert_handler = &urt_assert;
    else
        _assert_handler = handler;
}

// Frame-pointer backtrace straight to the console; safe in fault context. frame[0] returns into
// this function and frame[1] into its caller, so callers skip at least one.
version (BareMetal)
@noinline
void write_backtrace(size_t skip) nothrow @nogc
{
    import urt.internal.exception : capture_trace;

    void*[16] addrs = void;
    const n = capture_trace(addrs[]);
    if (n > skip)
        write_trace(addrs[skip .. n]);
}

version (BareMetal)
void write_trace(const(void*)[] addrs) nothrow @nogc
{
    import urt.driver.uart : uart0_puts;

    if (!addrs.length)
        return;
    uart0_puts("Backtrace (resolve with addr2line):\n");
    enum digits = size_t.sizeof * 2;
    char[4 + digits + 1] hex_buf = void;
    hex_buf[0 .. 4] = "  0x";
    hex_buf[$ - 1] = '\n';
    foreach (addr; addrs)
    {
        size_t v = cast(size_t)addr;
        foreach_reverse (j; 4 .. 4 + digits)
        {
            const ubyte nib = v & 0xF;
            hex_buf[j] = cast(char)(nib < 10 ? '0' + nib : 'a' + nib - 10);
            v >>= 4;
        }
        uart0_puts(hex_buf[]);
    }
}


private:

__gshared AssertHandler _assert_handler = &urt_assert;

@noinline
void urt_assert(string file, size_t line, string msg) nothrow @nogc
{
    if (msg.length == 0)
        msg = "Assertion failed";

    version (BareMetal)
    {
        import urt.driver.uart : uart0_puts;
        import urt.mem.temp : tconcat;

        uart0_puts(tconcat("\n*** ASSERT: ", msg, " at ", file, ':', line, '\n'));
        // frame[2] is the assert site inside the calling function
        write_backtrace(2);

        while (true) {}
    }
    else version (Espressif)
    {
        import urt.io : writef_to, WriteTarget;
        writef_to!(WriteTarget.stdout, true)("*** ASSERT: {2} at {0}:{1}", file, line, msg);
        import urt.internal.stdc.stdlib : exit;
        exit(-1);
    }
    else
    {
        import urt.io : write_to, WriteTarget;
        import urt.internal.exception : capture_trace, print_trace;

        debug
        {
            version (Windows)
                write_to!(WriteTarget.debugstring, true)(file, '(', line, "): ", msg);
        }

        // an assert fired from the print/resolve machinery below must not recurse.
        __gshared bool in_assert;
        if (!in_assert)
        {
            in_assert = true;
            write_to!(WriteTarget.stderr, true)("*** ASSERT: ", file, '(', line, "): ", msg);

            void*[32] addrs = void;
            const n = capture_trace(addrs[]);
            if (n > 1)
                print_trace(addrs[1 .. n]);
            in_assert = false;
        }

        debug
        {
            import urt.dbg;
            breakpoint();
        }
        else
        {
            import urt.internal.stdc.stdlib : exit;
            exit(-1);
        }
    }
}
