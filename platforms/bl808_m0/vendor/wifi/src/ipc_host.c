...
uint32_t ipc_host_get_status(struct ipc_host_env_tag *env)
{
    ...
    // Patched: upstream passed env->shared but reg_ipc_app.h declares ().
    // Other call site (line 335) already uses the no-arg form. Modern gcc
    // rejects the K&R mismatch; behaviour is unchanged.
