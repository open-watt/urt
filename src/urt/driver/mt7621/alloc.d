...
extern(C) void* malloc(size_t size)
    => alloc_impl(size, tlsf_align, MemFlags.none).ptr;
...
extern(C) void* calloc(size_t count, size_t size)
{
    if (size != 0 && count > size_t.max / size)
        return null;
    void[] m = alloc_impl(count * size, tlsf_align, MemFlags.none);
    if (m.ptr !is null)
        (cast(ubyte[])m)[] = 0;
    return m.ptr;
}
...
extern(C) void* realloc(void* ptr, size_t size)
{
    ...
    return realloc_impl(ptr[0 .. memsize_impl(ptr)], size, tlsf_align, MemFlags.none).ptr;
}
...
void heap_stats(out size_t total, out size_t used, out size_t peak, out size_t largest_free)
{
    ...
    used = _used;
    peak = _peak;
    tlsf_walk_pool(tlsf_get_pool(_tlsf), &walker_max_free, &largest_free);
}
...
extern(C) pool_t tlsf_get_pool(tlsf_t tlsf);
...
enum size_t tlsf_align = 8;
enum size_t tlsf_native_align = size_t.sizeof;
...
void init_pool()
{
    ...
    tlsf_add_pool(_tlsf, &__heap_start, _size);
}
...
void[] realloc_impl(void[] mem, size_t new_size, size_t alignment, MemFlags flags)
{
    ...
    if (is_uncached(mem.ptr))
        flags |= MemFlags.dma;
    if (alignment > tlsf_native_align || mem_is_dma(flags))
    {
