...
ifeq ($(COMPILER),ldc)
	mkdir -p $(OBJDIR)
	"$(DC)" $(DFLAGS) -unittest --fno-rtti --fno-moduleinfo -c -singleobj -of$(OBJDIR)/no-rtti.o src/urt/meta/nullable.d src/urt/mem/package.d src/urt/internal/sys/windows/winsock2.d
else
	$(error check-no-rtti requires COMPILER=ldc)
endif
