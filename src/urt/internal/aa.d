...
/// like urt.internal.traits.Unconst, but stripping inout, too
...
private:
    ...
    this(size_t sz /* = INIT_NUM_BUCKETS */) nothrow
    {
        ...
        key_sz = cast(uint) K.sizeof;
        val_sz = cast(uint) V.sizeof;
        val_off = cast(uint) talign(key_sz, V.alignof);

        enum ctflags = () {
            import urt.internal.traits;
            Impl.Flags flags;
            static if (__traits(hasPostblit, K))
                flags |= flags.key_has_postblit;
            static if (hasIndirections!K || hasIndirections!V)
                flags |= flags.has_pointers;
            return flags;
        } ();
        flags = ctflags;
    }
    ...
    immutable uint val_off;   // only for binary compatibility
    Flags flags;              // only for binary compatibility
    size_t delegate(scope ref const K) nothrow pure @nogc @safe hash_fn;

    enum Flags : ubyte
    {
        none = 0x0,
        key_has_postblit = 0x1,
        has_pointers = 0x2,
    }
    ...
    size_t find_slot_insert(size_t hash) const pure nothrow @nogc @safe
    {
    ...
    // lookup a key
    inout(Bucket)* find_slot_lookup(K2)(size_t hash, scope ref const K2 key) inout pure @safe nothrow
    {
        for (size_t i = hash & mask, j = 1;; ++j)
        {
    ...
    void resize(size_t ndim) pure nothrow @safe
    {
        ...
        buckets = alloc_buckets(ndim);

        foreach (ref b; obuckets[first_used .. $])
            if (b.filled)
                buckets[find_slot_insert(b.hash)] = b;
    ...
    void clear() pure nothrow
    {
        ...
        buckets[first_used .. $] = Bucket.init;
        deleted = used = 0;
        first_used = cast(uint) dim;
    }

    size_t calc_hash(K2)(ref K2 key) const nothrow pure @nogc @safe
    {
    ...
    {
        // CTFE has no allocator to call, so the interpreted path keeps `new`. That
        // costs nothing: a `new` reached only under __ctfe never asks for a TypeInfo,
        // only a runtime one does.
        if (__ctfe)
        ...
        static Bucket[] impl(size_t dim) nothrow @nogc @trusted
        {
            ...
            return defaultAllocator.allocArray!Bucket(dim);
        }
...
private size_t next_pow2(const size_t n) pure nothrow @nogc @safe
{
    import urt.internal.bitop : bsr;

    if (!n)
        return 1;

    const is_power_of_2 = !((n - 1) & n);
    return 1 << (bsr(n) + !is_power_of_2);
}
...
V* _aaGetX(K, V, K2)(auto ref scope V[K] a, auto ref K2 key, out bool found)
{
    ...
    immutable hash = aa.calc_hash(key2);

    // found a value => return it
    if (auto p = aa.find_slot_lookup(hash, key2))
    {
        found = true;
        return &p.entry.value;
    }

    auto pi = aa.find_slot_insert(hash);
    ...
    {
        aa.grow();
        pi = aa.find_slot_insert(hash);
        assert(aa.buckets[pi].empty);
    }

    // update search cache and allocate entry
    aa.first_used = min(aa.first_used, cast(uint)pi);
    ref p = aa.buckets[pi];
    p.hash = hash;
...
auto _aaDup(T : V[K], K, V)(T a)
{
    ...
    // copy the entries
    bool same_hash = aa.hash_fn == impl.hash_fn; // can be different if coming from template/rt
    foreach (b; aa.buckets[aa.first_used .. $])
    {
        if (!b.filled)
            continue;
        hash_t hash = same_hash ? b.hash : impl.calc_hash(b.entry.key);
        auto pi = impl.find_slot_insert(hash);
        auto p = &impl.buckets[pi];
        p.hash = hash;
        p.entry = new Entry!(K, V)(b.entry.key, b.entry.value);
        impl.first_used = min(impl.first_used, cast(uint)pi);
    }
...
V[K] _aaRehash(K, V)(V[K] a)
{
    auto aa = _toAA!(K, V)(a);
    if (!aa.empty)
        aa.resize(next_pow2(INIT_DEN * aa.length / INIT_NUM));
    return a;
}
...
Impl!(K, V)* _d_assocarrayliteralTX(K, V)(K[] keys, V[] vals)
{
    ...
    auto aa = new Impl!(K, V)(next_pow2(INIT_DEN * length / INIT_NUM));
    size_t duplicates = 0;
    foreach (i; 0 .. length)
    {
        immutable hash = aa.calc_hash(keys[i]);

        auto p = aa.find_slot_lookup!K(hash, keys[i]);
        if (p)
        {
...
bool _aaEqual(T : AA!(K, V), K, V)(scope T aa1, scope T aa2)
{
    ...
    bool same_hash = aa1.hash_fn == aa2.hash_fn; // can be different if coming from template/rt
    // compare the entries
    foreach (b1; aa1.buckets[aa1.first_used .. $])
    {
        if (!b1.filled)
            continue;
        hash_t hash = same_hash ? b1.hash : aa2.calc_hash(b1.entry.key);
        auto pb2 = aa2.find_slot_lookup!K(hash, b1.entry.key);
...
AARange!(K, V) _aaRange(K, V)(V[K] a)
{
    ...
    foreach (i; aa.first_used .. aa.dim)
    {
...
// Disabled: uses core.memory/GC and AA syntax that can't resolve inside
// this module when compiled as source (circular object ↔ newaa import).
...
AA!(K, V) makeAA(K, V)(V[K] src) @trusted
{
    ...
    auto impl = _d_assocarrayliteralTX!(K, V)(keys, values);
    return AA!(K, V)(impl);
}

// Disabled: AA-indexing lowering inside this module creates a circular
// import (object → core.internal.newaa → object) that prevents template
// resolution when compiled as source rather than a pre-compiled library.
version (none) unittest
{
    static struct Foo
    {
