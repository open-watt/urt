...
struct bl_mod_params bl_mod_params = {
    /* common parameters */
    /* The BL808 AP path currently emits malformed HT Operation IEs when HT is
     * enabled (sniffer shows garbage/bleed in IE 61). Keep AP/STA in legacy
     * 11bg mode until the HT template state is understood. */
