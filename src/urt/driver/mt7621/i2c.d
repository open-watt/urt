...
I2cError address_phase(ref const I2cTransfer transfer, bool read, MonoTime deadline)
{
    ...
    uint addr;
    size_t len;
    if (transfer.address_mode == I2cAddressMode.ten_bit)
    {
        addr = (0xF0 | ((transfer.address >> 7) & 6) | read) | (transfer.address & 0xFF) << 8;
        len = 2;
    }
    else
    {
        addr = transfer.address << 1 | read;
        len = 1;
    }
    mmio_write(i2c_base + sm0d0, addr);
    if (!command(ctl1_write, len, deadline))
        return I2cError.timeout;
    return acked(1) ? I2cError.none : I2cError.nack;
}
