...
bool wifi_open(uint port, ref const WifiConfig cfg)
{
    if (_opened)
        return false;
...
bool wifi_set_mode(uint port, WifiMode mode)
{
    // WifiMode enum values match ESP-IDF wifi_mode_t: 0=none,1=sta,2=ap,3=apsta
    return esp_wifi_set_mode(cast(int)mode) == ESP_OK;
}

bool wifi_sta_configure(uint port, ref const WifiStaConfig cfg)
{
...
bool wifi_sta_connect(uint port)
{
    _evt_sta_connected = false;
    _evt_sta_disconnected = false;
...
bool wifi_sta_disconnect(uint port)
{
    return esp_wifi_disconnect() == ESP_OK;
}

bool wifi_ap_configure(uint port, ref const WifiApConfig cfg)
{
    char[33] ssid_z = 0;
    char[65] pw_z = 0;
...
size_t wifi_ap_get_clients(uint port, WifiStaInfo[] buf)
{
...
bool wifi_scan_start(uint port, ref const WifiScanConfig cfg)
{
    // TODO: esp_wifi_scan_start
    return false;
}

void wifi_scan_stop(uint port)
{
    // TODO: esp_wifi_scan_stop
}

size_t wifi_scan_get_results(uint port, WifiScanResult[] buf)
{
...
int wifi_tx(uint port, WifiVif vif, const(ubyte)[] data)
{
    if (data.length == 0)
        return -1;
    return esp_wifi_internal_tx(cast(int)vif, cast(void*)data.ptr, cast(ushort)data.length) == ESP_OK ? 0 : -1;
}

void wifi_set_rx_callback(uint port, WifiRxCallback cb)
{
    _rx_cb = cb;
...
bool wifi_get_mac(uint port, WifiVif vif, ref ubyte[6] mac)
{
    // ESP_MAC_WIFI_STA=0, ESP_MAC_WIFI_SOFTAP=1
    return esp_read_mac(mac.ptr, cast(int)vif) == ESP_OK;
}

ubyte wifi_get_channel(uint port)
{
    ubyte primary = void;
    int second = void;
...
byte wifi_get_rssi(uint port)
{
    // TODO: esp_wifi_sta_get_ap_info -> rssi
    return -127;
}

bool wifi_set_tx_power(uint port, byte power_dbm)
{
...
void wifi_set_event_callback(uint port, WifiEventCallback cb)
{
    _event_cb = cb;
}
