...
import urt.driver.mt7621 : mmio_read, mmio_write, sysc_rstctrl, sysctl_base;
...
// A transfer that never finished leaves the controller busy, and the NOR window reads through it too.
void recover()
{
    foreach (i; 0 .. 1_000_000)
    {
        if (!(mmio_read(spi_trans) & trans_busy))
            return;
    }
    mmio_write(sysctl_base + sysc_rstctrl, mmio_read(sysctl_base + sysc_rstctrl) | rst_spi);
    mmio_write(sysctl_base + sysc_rstctrl, mmio_read(sysctl_base + sysc_rstctrl) & ~rst_spi);
}
