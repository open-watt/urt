...
version (Espressif)
{
    ...
    private extern(C) nothrow @nogc
    {
        ...
        void MD5Update(MD5Context* ctx, const ubyte* buf, uint len);
        void MD5Final(ubyte* digest, MD5Context* ctx);
    }
    ...
    void md5_update(ref MD5Context ctx, const void[] data)
    {
        MD5Update(&ctx, cast(const ubyte*)data.ptr, cast(uint)data.length);
    }

    ubyte[16] md5_finalise(ref MD5Context ctx)
    {
        ubyte[16] digest;
        MD5Final(digest.ptr, &ctx);
        return digest;
    }
}
else
...
unittest
{
    ...
    md5_update(ctx, "a");
    digest = md5_finalise(ctx);
    assert(digest == HexDecode!"0cc175b9c0f1b6a831c399e269772661");

    md5_init(ctx);
    md5_update(ctx, "abc");
    digest = md5_finalise(ctx);
    assert(digest == HexDecode!"900150983cd24fb0d6963f7d28e17f72");

    md5_init(ctx);
    md5_update(ctx, "message digest");
    digest = md5_finalise(ctx);
    assert(digest == HexDecode!"f96b697d7cb7938d525a2f31aaf161d0");

    // progressive: split across multiple updates
    md5_init(ctx);
    md5_update(ctx, "Hello, ");
    md5_update(ctx, "World!");
    digest = md5_finalise(ctx);
    assert(digest == HexDecode!"65a8e27d8879283831b664bd8b7f0ad4");

    // data spanning multiple 64-byte blocks
    md5_init(ctx);
    md5_update(ctx, "12345678901234567890123456789012345678901234567890123456789012345678901234567890");
    digest = md5_finalise(ctx);
    assert(digest == HexDecode!"57edf4a22be3c955ac49da2e2107b67a");
}
