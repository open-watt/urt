...
//     key_rsc (8)                    -- group key sequence, least significant octet first
...
private ushort be_u16(const(ubyte)* p) pure
    => cast(ushort)((cast(ushort)p[0] << 8) | p[1]);
...
size_t encode_eapol_key(ubyte[] dst, ubyte eapol_version, ubyte descriptor_type, ushort key_info, ushort key_length, const(ubyte)[eapol_key_replay_len] replay, const(ubyte)[eapol_key_nonce_len] nonce, const(ubyte)[eapol_key_rsc_len] rsc, const(ubyte)[] key_data)
{
...
bool parse_gtk_kde(const(ubyte)[] data, ubyte[] out_gtk, ref size_t out_len, ref ubyte out_key_id)
{
    ...
    while (off + 2 <= data.length)
    {
        ...
        if (id == 0xDD && len >= 6)
        {
            ...
            if (data[off + 2] == 0x00 && data[off + 3] == 0x0F && data[off + 4] == 0xAC && data[off + 5] == 0x01)
            {
