...
extern(C) void sys_init()
{
    ...
    irq_init();
    counter_init(cpu_hz);
    mmio_write(gpio_base + 0x00, mmio_read(gpio_base + 0x00) | led);
...
void probe_heartbeat(ulong us)
{
    enum ulong reset_after_us = 600_000_000;
    if (us > reset_after_us)
    {
        import urt.driver.reset : system_reset;
        system_reset();
    }
    immutable bool on = ((us >> 19) & 1) != 0;
    if (on == _led_on)
        return;
    _led_on = on;
    mmio_write(gpio_base + (on ? 0x30 : 0x40), led);
}
...
void spin_us(uint us)
{
    immutable uint ticks = ((cpu_hz ? cpu_hz : 880_000_000) / 2 / 1_000_000) * us;
    uint start;
    asm nothrow @nogc { ".set push; .set noat; mfc0 %0, $9; .set pop" : "=r"(start); }
    uint now;
    do
        asm nothrow @nogc { ".set push; .set noat; mfc0 %0, $9; .set pop" : "=r"(now); }
    while (now - start < ticks);
}

private:

__gshared bool _led_on;
