...
extern (C) int main(int argc, char** argv) nothrow @nogc @trusted
{
    ...
    // Convert C argv to D string[] (zero-copy, just wrap pointers)
    ...
    // Run unit tests (in unittest builds) and exit
    version (unittest)
    {
        ...
        exit(executed > 0 && passed == executed ? 0 : 1);
    }

    int result = call_dmain(d_args);

    // Run module destructors in reverse order
    run_module_dtors(modules);
