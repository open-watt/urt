...
// Removed: #include <wifi_mgmr_ext.h>  -- pulls lwIP/wifi_hosal we don't link.
#include "lmac_mac.h"
...
// Patched: was a bare definition (`} cfg_start_req_u_tlv_t;`), relied on
// pre-gcc-10 -fcommon to merge per-TU defs. Modern gcc refuses to make a
// var with a flexible array member COMMON. Header declares the type +
// extern; the single definition lives in src/bl_globals.c.
struct cfg_start_req_u_tlv_s
{
    /// TASK
    uint32_t task;
    /// ELEMENT
...
extern struct cfg_start_req_u_tlv_s cfg_start_req_u_tlv_t;
